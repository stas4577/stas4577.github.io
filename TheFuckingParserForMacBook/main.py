...
def get_fix_links (html):
	links = html.select('.infobox li a')
	return links
...
link = 'https://ifix-it.ru/macbook-unibody'
page = get_html(link)
...
for i in range(len(fix_links)):
	...
	if info == False:
		...
		if title == False:
			print ('Что то не так с заголовком')
			continue
		...
		ready = open('C:\\Users\\Стас\\Desktop\\работа\\fix — копия\\TheFuckingParserForMacBook\\' + folder + '\\index.txt','w',encoding="utf-8")
		main_text = main_text.replace('$next','')
		ready.write(main_text)
		ready.close()
		os.rename(os.getcwd() +'\\' + folder + '\\index.txt',os.getcwd() + '\\'+ folder + '\\index.kit')
