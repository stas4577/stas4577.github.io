<?php 
include 'bx24.php';
$imail="info@akvastroigrand-pools.ru"; //Email (можно через запятую - email1, email2, email3)
...
if (
  ...
  $post = array(
    ...
    'naznachenie'=> 'Назначение и расположение: ',
...
function frarr($arr){
  ...
  $header = 'Content-type: text/plain; charset=UTF-8' . PHP_EOL . 
            'From: ' . 'no-reply@akva-sg.ru' . PHP_EOL . 
...
// сообщение не отправилось
  ...
  if (mail($imail, $title, $message, $header) == 1) {
    mail("akvastroigrand.pools@yandex.ru", $title, $message, $header);
    mail("sergeykozin1@gmail.com", $title, $message, $header);
    mail("9932202@mail.ru", $title, $message, $header);

    $title = 'Заявка с сайта akvastroigrand-pools.ru';
